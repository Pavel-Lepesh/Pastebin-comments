...
from app.exceptions.exceptions import ObjectNotFound, ParentConflict, ParentCommentNotFoundError
...
class TestIntegrationServices:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (CommentScheme(body="test body", parent_id=PydanticObjectId("661eb8d5b4a2f431dcb8f1d1")),
             1,
             "hash_1",
             True,
             None
             ),
            (CommentScheme(body="test body", parent_id=PydanticObjectId("661eb8d5b4a2f431dcb8f100")),
             1,
             "hash_1",
             False,
    ...
    async def test_create_comment(
        ...
        if success:
            comment = await CommentService.create_comment(mock_comment_data, user_id, mock_hash_link)
            assert comment
            assert comment.body == mock_comment_data.body
            assert comment.parent_id == mock_comment_data.parent_id
        else:
            with pytest.raises(exception):
                await CommentService.create_comment(mock_comment_data, user_id, mock_hash_link)

    @pytest.mark.parametrize(
        "mock_hash_link, mock_page, mock_limit, count, success",
        [
            ("hash_3", 1, 10, 2, True),
            ("unexpected", 1, 10, 2, False)
        ]
    )
    async def test_get_note_all_comments(
        ...
        if success:
            comments = await CommentService.get_note_all_comments(mock_hash_link, mock_page, mock_limit)
            assert comments
    ...
    async def test_get_comment_by_id(
        ...
        if success:
            comment = await CommentService.get_comment_by_id(mock_comment_id, fetch_children)
            assert comment
    ...
    @pytest.mark.parametrize(
        ...
        [
            (PydanticObjectId("661eb8d5b4a2f431dcb8f1d1"), CommentUpdateScheme(body="new body"), True),
            (PydanticObjectId("661eb8d5b4a2f431dcb8f100"), CommentUpdateScheme(body="new body"), False),
        ]
    )
