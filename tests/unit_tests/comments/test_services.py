...
class TestServices:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                CommentScheme(
                    body="string",
                    parent_id=PydanticObjectId("67fb6ba0539e68884a03a031"),
                ),
                1,
                "some_hash",
                True,
            ...
            (
                CommentScheme(
                    body="string",
                    parent_id=PydanticObjectId("67fb6ba0539e68884a03a042"),
                ),
                1,
                "some_hash",
                False,
    ...
    async def test_create_comment(
        self, mock_comment_data, mock_user_id, mock_hash_link, success, exc, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            CommentsDAO, "get_comment_by_id", self.mock_get_comment_by_id, raising=True
        )
        monkeypatch.setattr(
            CommentsDAO, "insert_comment", mock_insert_comment, raising=True
        )
        monkeypatch.setattr(
            CommentsDAO, "update_parent_comment", mock_update_comment, raising=True
        )

        if success:
            result = await CommentService.create_comment(
                mock_comment_data, mock_user_id, mock_hash_link
            )
            assert result
    ...
    async def test_get_note_all_comments(
        self, mock_hash_link, mock_page, mock_limit, success, monkeypatch
    ):
        ...
        monkeypatch.setattr(
            CommentsDAO,
            "get_comments_by_hash_link",
            mock_get_comments_by_hash_link,
            raising=True,
        )

        if success:
            result = await CommentService.get_note_all_comments(
    ...
    async def test_get_comment_by_id(
        self, mock_comment_id, mock_children, success, monkeypatch
    ):
        monkeypatch.setattr(
            CommentsDAO, "get_comment_by_id", self.mock_get_comment_by_id, raising=True
        )
        ...
        if success:
            result = await CommentService.get_comment_by_id(
                mock_comment_id, mock_children
            )
            assert result
    ...
    async def test_update_comment(
        self, mock_comment_id, mock_user_id, success, monkeypatch
    ):
        async def mock_update_comment(comment, body):
            return None
        ...
        monkeypatch.setattr(
            CommentsDAO, "update_comment", mock_update_comment, raising=True
        )
        ...
        if success:
            result = await CommentService.update_comment(
                mock_comment_id, data, mock_user_id
            )
            assert not result
        else:
            with pytest.raises(ObjectNotFound):
    ...
    async def test_delete_comment(
        ...
        monkeypatch.setattr(
            CommentsDAO, "delete_comments", mock_delete_comments, raising=True
        )
