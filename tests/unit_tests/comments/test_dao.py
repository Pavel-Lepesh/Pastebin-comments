...
class TestDAO:
    async def test_insert_comment(self, mocker):
        comment_data = CommentInsertScheme(
            note_hash_link="test_hash", user_id=1, body="Test body", parent_id=None
        )

        fake_comment = MagicMock()
        mock_comment_class = mocker.patch(
            "app.comments.dao.Comment", return_value=fake_comment
        )
        mock_insert = mocker.patch.object(
            fake_comment, "insert", new_callable=AsyncMock
        )
    ...
    async def test_get_comment_by_id(self, mocker):
        ...
        mock_get.assert_called_once_with(
            document_id=mock_data["comment_id"], fetch_links=mock_data["fetch_children"]
        )
        assert result == fake_comment

    async def test_get_comments_by_hash_link(self, mocker):
        mock_data = {"note_hash_link": "some_hash", "skip": 0, "limit": 10}

        fake_comments_list = [MagicMock()]
        mock_find_result = MagicMock()
        mock_find_result.to_list = AsyncMock(return_value=fake_comments_list)
        mock_find = mocker.patch(
            "app.comments.dao.Comment.find", return_value=mock_find_result
        )
    ...
    async def test_update_parent_comment(self, mocker):
        ...
        await CommentsDAO.update_parent_comment(
            fake_parent_comment, fake_children_comment
        )
